...
for t_current in times_for_qsp: 
    # 1. Tạo target_state V(t_current)^dagger 
    target_unitary_at_t_circuit = time_dependent(num_qubits, H_time, t_current)
    target_unitary_matrix = Operator(target_unitary_at_t_circuit).data
    print(f"Ma trận Target Unitary U_target(t) tại t = {t_current:.4f}:")
    print(target_unitary_matrix)
    ...
    # print(f"Ma trận U(theta) tại t = {t_current:.4f}:")
